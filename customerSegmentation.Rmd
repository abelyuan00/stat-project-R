...
After the result from above plot, we can choose the kluster number for the k-means. And we can seperate the customer to 4 clusters based on their income level, the car price and car company
```{r}
...
# Assuming `summary_stats` is your data frame and you want to exclude non-numeric columns for PCA
data_for_pca <- summary_stats %>%
  select_if(is.numeric) %>%
  na.omit()  # Removing rows with NA values, consider more sophisticated imputation if necessary
...
# Example: Using the first two principal components
pc_data <- as.data.frame(pca_result$x[, 1:2])
# Assuming `Average_Price` is the outcome and is part of your original `summary_stats` data frame
...
# For cross-validation, you might consider using the `train` function from the `caret` package
